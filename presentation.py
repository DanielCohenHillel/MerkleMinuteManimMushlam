...
if "manim" in sys.modules:
    ...
    raise ImportError(
        "This script must be run with either `manim` or `manimgl`")
...
class MerkleTree:
    ...
    def create(self):
        self.nodes = [RoundedRectangle(corner_radius=0.5)
...
class Presentation(MovingCameraScene):
    def construct(self):
        ...
        left = RoundedRectangle(
            corner_radius=0.2, width=1.5, height=1, fill_opacity=1)
        ...
        dad = RoundedRectangle(
            corner_radius=0.2, width=1.35, height=1.05, fill_opacity=1)
        dad.move_to(dad_pos)
        dad_left = dad.copy()

        dad_new = RoundedRectangle(
            corner_radius=0.2, width=1.5, height=1, fill_opacity=1)
        ...
        ag1 = AnimationGroup(left_new.animate.move_to(
            left_midpoint), right_new.animate.move_to(right_midpoint))
        ag2 = AnimationGroup(Transform(left_new, dad_left),
                             Transform(right_new, dad))

        arrow_grow_ag = AnimationGroup(
            GrowArrow(left_arrow), GrowArrow(right_arrow), run_time=1.1)
        ...
        self.play(Transform(dad, dad_new),
                  rate_func=rate_functions.ease_in_sine, run_time=0.2)
        ...
        dad_dad = RoundedRectangle(
            corner_radius=0.2, width=1.5, height=1, fill_opacity=1)
        dad_dad.move_to(dad_dad_pos)

        dad_bro = RoundedRectangle(
            corner_radius=0.2, width=1.5, height=1, fill_opacity=1)
        ...
        dad_merge_ag = AnimationGroup(
            Transform(dad_bro_new, dad_dad), Transform(daddy_new, dad_dad_tmp))

        dad_left_arrow = Arrow(dad_pos, dad_dad_pos, buff=0.85)
        dad_right_arrow = Arrow(dad_bro_pos, dad_dad_pos, buff=0.85)
        dad_arrow_ag = AnimationGroup(
            GrowArrow(dad_left_arrow), GrowArrow(dad_right_arrow), run_time=1.1)
...
class Buffers(MovingCameraScene):
    def merge_into_dad(self, left, right, text, font_size=DEFAULT_FONT_SIZE, dad_height=2, z_index=1, scaling=1, arrows_buff=0.8):  # -> dad, AnimationGroup
        ...
        dad = VGroup(
            RoundedRectangle(corner_radius=0.2, width=dad_width,
                             height=1, fill_opacity=1, stroke_opacity=0),
            dad_text
        )
        ...
        merge_ag = AnimationGroup(TransformFromCopy(
            left, dad), TransformFromCopy(right, dad_tmp))
        arrows_ag = AnimationGroup(
            GrowArrow(left_arrow), GrowArrow(right_arrow))

        res_ag = AnimationGroup(AnimationGroup(
            merge_ag, arrows_ag), lag_ratio=0.25)
    ...
    def create_buffer(self, texts, fill_color, stroke_color, text_color, title_text):
        ...
        buff_height = 1
        text_height_buf = 0.2
        leaves_buffer = RoundedRectangle(corner_radius=0.1, width=leaves_buf_width,
                                         height=buff_height + roof_buf, stroke_width=6, color=stroke_color)

        eff_leaf_width = leaf_width + leaf_buf
        leaves_arr = [
            VGroup(
                RoundedRectangle(corner_radius=0.2, width=leaf_width, height=buff_height,
                                 fill_opacity=1, stroke_opacity=0, color=fill_color),
        ...
        for i, leaf in enumerate(leaves_arr):
            ...
            leaf.move_to([rect_left + left_buf + leaf_width /
                         2 + i * eff_leaf_width, 0, 0])

        self.play(GrowFromCenter(leaves_buffer))

        title = Text(title_text, color=text_color, font_size=36)
        title.move_to([-leaves_buf_width/2 + title.width/2, -
                      buff_height/2 - title.height/2 - text_height_buf, 0])

        self.play(AnimationGroup(*[GrowFromCenter(l)
                  for l in leaves_arr], lag_ratio=0.15), Write(title))
        behind_leaves_arr = [
        ...
        leaves_vgroup = VGroup(
            *(leaves_arr + behind_leaves_arr + [leaves_buffer, title]))
    ...
    def explain(self, obj, text, color=RED, z_index=10, wait_time=1):
        ...
        shade_background = Rectangle(
            width=1000, height=1000, fill_opacity=0.9, color=BLACK)
    ...
    def explain_noanim(self, obj, text, color=RED, lag_ratio=2, center=False):
        ...
        fadeouts_ag = AnimationGroup(
            FadeOut(shade_background), FadeOut(explain))
    ...
    def end_credits(self, texts, color=WHITE, lag_ratio=7.0):
        ...
        credits_ag = AnimationGroup(*[FadeIn(line)
                                    for line in credits], lag_ratio=0.75)
        fadeins_ag = AnimationGroup(FadeIn(shade_background), credits_ag)
        fadeouts_ag = AnimationGroup(
            FadeOut(shade_background), FadeOut(credits))
    ...
    def full_tree_construct(self):
        ...
        leaves_texts = ['a', 'b']
        leaves_vgroup, _, _, _, _ = self.create_buffer(
            leaves_texts, GREEN_A, GREEN, GREEN_E, "leaves")
        self.play(leaves_vgroup.animate.move_to(
            [-6 + leaves_vgroup.width/2, bufs_dist, 0]).scale(0.8))

        proof_text = ['A', 'B']
        proof_vgroup, _, _, _, _ = self.create_buffer(
            proof_text, YELLOW_A, YELLOW, YELLOW_E, "proof")
        self.play(proof_vgroup.animate.move_to(
            [-6 + proof_vgroup.width/2, 0, 0]).scale(0.8))

        struct_text = ['0', '0', '1']
        struct_vgroup, _, _, _, _ = self.create_buffer(
            struct_text, RED_A, RED, RED_E, "proof flags")
        self.play(struct_vgroup.animate.move_to(
            [-6 + struct_vgroup.width/2, -bufs_dist, 0]).scale(0.8))

        hashes_text = ['', '', '']
        hashes_vgroup, _, _, _, _ = self.create_buffer(
            hashes_text, GREY_D, GREY, GREY_C, "hashes")
        self.play(hashes_vgroup.animate.move_to(
            [-6 + hashes_vgroup.width/2 + leaves_vgroup.width, bufs_dist, 0]).scale(0.8))

        arrows_top_buf = 0.2
        leaves_arrow = Arrow([0, 1, 0], [0, 0, 0],
                             max_stroke_width_to_length_ratio=10, color=GREEN)
        proof_arrow = Arrow([0, 1, 0], [0, 0, 0],
                            max_stroke_width_to_length_ratio=10, color=YELLOW)
        bits_arrow = Arrow([0, 1, 0], [0, 0, 0],
                           max_stroke_width_to_length_ratio=10, color=RED)

        arrows_start = np.array(
            [-6 + 0.25, leaves_vgroup.height/2 + leaves_arrow.height/2 + arrows_top_buf, 0])
        ...
        self.play(GrowArrow(leaves_arrow), GrowArrow(
            proof_arrow), GrowArrow(bits_arrow))
        leaves_hashes_xs = [i.get_center()[0] - i.width / 2 for i in leaves_vgroup[:2]] + [
            i[0].get_center()[0] - i[0].width / 2 for i in hashes_vgroup[:3]]
        ...
        self.play(l2_left_left.animate.move_to(
            [0, -2, 0]), la_mtx(leaves_arrow, 1))

        # explain the bits before continueing
        self.explain(struct_vgroup[0], "Take from proof list")

        self.play(l2_left_right.animate.move_to(
            [2, -2, 0]), la_mtx(proof_arrow, 1), la_mtx(bits_arrow, 1))

        scaling = 0.8
        arrows_buff = 0.5

        l1_left, l1_left_ag = self.merge_into_dad(
            l2_left_left, l2_left_right, "aA", scaling=scaling, arrows_buff=arrows_buff)
        ...
        self.play(l2_right_left.animate.move_to(
            [4, -2, 0]), la_mtx(leaves_arrow, 2))

        self.explain(struct_vgroup[1], "Take from proof list")

        self.play(l2_right_right.animate.move_to(
            [6, -2, 0]), la_mtx(proof_arrow, 2), la_mtx(bits_arrow, 2))

        l1_right, l1_right_ag = self.merge_into_dad(
            l2_right_left, l2_right_right, "bB", scaling=scaling, arrows_buff=arrows_buff)
        ...
        dad, dad_ag = self.merge_into_dad(
            l1_left, l1_right, "aAbB", font_size=24, scaling=scaling, arrows_buff=arrows_buff)
        self.play(dad_ag, hashes_0.animate.set_opacity(0.2), hashes_1.animate.set_opacity(
            0.2), la_mtx(leaves_arrow, 4), la_mtx(bits_arrow, 3))
    ...
    def three_tree_construct(self):
        ...
        hashes_text = ['', '']
        hashes_vgroup, _, hashes_arr, _, _ = self.create_buffer(
            hashes_text, GREY_D, GREY, GREY_C, "hashes")
        self.play(hashes_vgroup.animate.move_to(
            [-6 + hashes_vgroup.width/2 + leaves_vgroup.width, bufs_dist, 0]).scale(0.8))
        ...
        leaves_hashes_xs = [i.get_center()[0] - i.width / 2 for i in leaves_vgroup[:2]] + [
            i[0].get_center()[0] - i[0].width / 2 for i in hashes_vgroup[:2]]
        ...
        self.play(l2_left_right.animate.move_to(
            [2, -2, 0]), la_mtx(leaves_arrow, 2), la_mtx(bits_arrow, 1))

        l1_left, l1_left_ag = self.merge_into_dad(
            l2_left_left, l2_left_right, "ab", scaling=scaling, arrows_buff=arrows_buff)
        self.play(l1_left_ag)
        hashes_0 = l1_left.copy().set_z_index(2)
        self.play(hashes_0.animate.move_to(hashes_vgroup[0]))
        ...
        self.play(hashes_0.animate.move_to(
            l1_left.get_center()), la_mtx(leaves_arrow, 3))

        self.explain(struct_vgroup[1], "Take from proof list")

        l1_right = proof_vgroup[0]
        l1_right.set_z_index(3)
        self.play(l1_right.animate.move_to([4, 0, 0]), la_mtx(
            proof_arrow, 1), la_mtx(bits_arrow, 2))

        root, root_ag = self.merge_into_dad(
            l1_left, l1_right, "abA", font_size=30, scaling=scaling, arrows_buff=arrows_buff)
        self.play(root_ag)
    ...
    def malicious_tree_construct(self):
        ...
        leaves_hashes_xs = [i.get_center()[0] - i.width / 2 for i in leaves_vgroup[:1]] + [
            i[0].get_center()[0] - i[0].width / 2 for i in hashes_vgroup[:2]]
        ...
        l1_left, l1_left_ag = self.merge_into_dad(
            l2_left_left, l2_left_right, "a0", scaling=scaling, arrows_buff=arrows_buff)
        self.play(l1_left_ag)
        ...
        self.play(hashes_1.animate.move_to(np.array(l1_left.get_center()) +
                  np.array([-0.7, 1, 0])), leaves_arrow.animate.shift([1, 0, 0]))

        self.explain(struct_vgroup[1], "Take from proof list")

        l1_right = proof_vgroup[0]
        l1_right.set_z_index(3)
        self.play(l1_right.animate.move_to(np.array(hashes_1.get_center()) +
                  np.array([2, 0, 0])), la_mtx(proof_arrow, 1), la_mtx(bits_arrow, 2))

        root, root_ag = self.merge_into_dad(
            hashes_1, l1_right, "0A", scaling=scaling, arrows_buff=arrows_buff)
        self.play(root_ag)
        ...
        end_credits_ag = self.end_credits(["The calculated root does not depend on the input leaves!",
                                          "If there's ever a child of the root is 0,", f"an attacker could <span fgcolor=\"{RED}\">validate malicious leaves!</span>"])
        self.play(end_credits_ag)

    def build_tree(self):
        fastpace_run_time = 0.4

        leaves_info = [('a', BLUE_E), ('b', RED_E),
                       ('c', GREEN_E), ('d', YELLOW_E)]
        leaves = [
            VGroup(
                RoundedRectangle(corner_radius=0.2, width=1,
                                 height=1, fill_opacity=1, stroke_opacity=0),
        ...
        self.play(GrowFromCenter(leaves[0]), GrowFromCenter(
            leaves[1]), run_time=fastpace_run_time)

        l1_left_text = f'H(<span fgcolor="{leaves_info[0][1]}">a</span>,<span fgcolor="{leaves_info[1][1]}">b</span>)'
        l1_left, l1_left_ag = self.merge_into_dad(
            leaves[0], leaves[1], l1_left_text, font_size=30, dad_height=2.5)
        ...
        self.play(GrowFromCenter(leaves[2]), GrowFromCenter(
            leaves[3]), run_time=fastpace_run_time)

        l1_right_text = f'H(<span fgcolor="{leaves_info[2][1]}">c</span>,<span fgcolor="{leaves_info[3][1]}">d</span>)'
        l1_right, l1_right_ag = self.merge_into_dad(
            leaves[2], leaves[3], l1_right_text, font_size=30, dad_height=2.5)
        self.play(l1_right_ag, run_time=fastpace_run_time)

        root_text = f'H({l1_left_text},{l1_right_text})'
        root, root_ag = self.merge_into_dad(
            l1_left, l1_right, root_text, font_size=30, dad_height=2.5)
        ...
        explain_leaf_ag = self.explain_noanim(
            leaves[2], "Leaf to verify", GREEN)
        self.play(explain_leaf_ag, leaves[2][0].animate.set_color(GREEN_A))
        leaves[2].set_z_index(3)

        l1_left.set_z_index(5)
        explain_proof_ag = self.explain_noanim(
            VGroup(leaves[3], l1_left), "The proof", YELLOW, center=True)
        self.play(explain_proof_ag, leaves[3][0].animate.set_color(
            YELLOW_A), l1_left[0].animate.set_color(YELLOW_A))
        ...
        explain_text = Text(
            "everything you need to verify the leaf", font_size=36, color=WHITE)
        ...
        shade_background = Rectangle(
            width=1000, height=1000, fill_opacity=0.93, color=BLACK)
        shade_background.move_to([-50, 50, 0])
        shade_background.set_z_index(9)
        ...
        l1_right_text = f'H(<span fgcolor="{leaves_info[2][1]}">c</span>,<span fgcolor="{leaves_info[3][1]}">d</span>)'
        l1_right, l1_right_ag = self.merge_into_dad(
            leaves[2], leaves[3], l1_right_text, font_size=30, dad_height=2.5, z_index=10)
        self.play(l1_right_ag)

        root_text = f'H({l1_left_text},{l1_right_text})'
        root, root_ag = self.merge_into_dad(
            l1_left, l1_right, root_text, font_size=30, dad_height=2.5, z_index=10)
        self.play(root_ag)
